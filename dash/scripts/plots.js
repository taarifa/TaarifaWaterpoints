function init(){
    //load the basic waterpoint stats
    //TODO: loaded only once and shared for ease of use but could be problematic
    //as it becomes clear what endpoints we need this will probably be pushed into the plot functions
    d3.json("http://localhost:5000/api/waterpoints/stats_by_district", function(error, wpdata) {
        drawPlots(wpdata);
    });
}
...
function drawPlots(wpdata){
    doughnut("#doughnut");
    //TODO strange alignment problem
    //parallelAxis("parallelAxis",wpdata);

    //TODO data is shared & should run in parallel
    statusPerDistrict("#statusPerDistrict",wpdata);
...
function doughnut(selector){
    ...
    d3.json("http://localhost:5000/api/waterpoints/status", function(error, data) {
        ...
        var sum = d3.sum(data, function(x){return x.count;});
...
function parallelAxis(selector,data){
    ...
    var color = function(d) { return colors[d.district]; };
    ...
    var g = svg.selectAll(".dimension")
        ...
        .call(d3.behavior.drag()
                ...
                .on("drag", function(d) {
                    ...
                    dims.sort(function(a, b) { return position(a) - position(b); });
                    x.domain(dims);
                ...
                function path(d) {
                    return line(dims.map(function(p) { return [position(p), y[p](d[p])]; }));
                }
                ...
                function brush() {
                    var actives = dims.filter(function(p) { return !y[p].brush.empty(); }),
...
function statusPerDistrict(selector,data){

    data.forEach(function(district){
        var y0 = 0;
        //status type is not always in the same order due to mongo, sort here
        district.waterpoints = _.sortBy(district.waterpoints,"status");
        district.waterpoints.forEach(function(x){ x.y0 = y0; x.y1 = (y0 += x.count); });
    });
    ...
    var margin = {top: 20, right: 20, bottom: 80, left: 55},
        width = w - margin.left - margin.right,
        height = h - margin.top - margin.bottom;
    ...
    state.selectAll("rect")
        .data(function(d) { return d.waterpoints; })
        .enter().append("rect")
...
function spendPerDistrict(selector,data){
    ...
    var state = svg.selectAll(".district")
        ...
        .append("rect")
        .attr("width", x.rangeBand())
        .attr("y", function(d) { return y(0)-y(d.spend); })
        .attr("height", function(d) { return y(d.spend); })
...
function statusOverTime(selector){
    ...
    _.range(2000,2015).forEach(function(i){
        data.push({date: i.toString(), count: 1000, status: [
            {status: "Functional", count: Math.random()*500},
            {status: "Not Functional", count: Math.random()*500}
        ]});
    });
    ...
    var stati = stack(color.domain().map(function(name) {
        ...
        values: data.map(function(d) {
            return {date: d.date, y: _.find(d.status, function(x){return x.status == name;}).count};
        })
        };
    }));
...
function spendVsStatus(selector,wpdata){
    ...
    wpdata.forEach(function(x){
        var functional = _.find(x.waterpoints, function(x){ return x.status == "Functional";});
        data.push(
    ...
    svg.selectAll(".dot")
        ...
        .attr("class", "dot")
        .attr("r", 3.5)
        .attr("cx", function(d){ return x(d.spend); })
        .attr("cy", function(d){ return y(d.functional); })
        .style("fill", function(d){ return color(d.district);})
        .on("mouseover", function(d) {
            tooltip.transition()
            .duration(100)
            .style("opacity", .9);
        tooltip.html(d.district + "<br/> (" + d.spend.toPrecision(3)
            + ", " + d.functional.toPrecision(3) + ")")
...
function districtRanking(selector,data){
    ...
    var comparator = function(a,b){
        var af = _.find(a.waterpoints, function(x){ return x.status == "Functional";}).count;
        var bf = _.find(b.waterpoints, function(x){ return x.status == "Functional";}).count;
